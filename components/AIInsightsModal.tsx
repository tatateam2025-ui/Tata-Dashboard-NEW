...
export const AIInsightsModal: React.FC<AIInsightsModalProps> = ({ isOpen, onClose, content, isLoading }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6">
      ...
      {/* Modal Content */}
      <div className="relative bg-white w-full max-w-2xl rounded-3xl shadow-2xl shadow-blue-900/20 overflow-hidden transform transition-all animate-in fade-in zoom-in duration-300">
            ...
            <div className="prose prose-slate max-w-none">
              ...
              <div className="mt-8 grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="bg-blue-50 p-4 rounded-2xl flex items-start gap-4">
                  ...
                  <div>
                    <h4 className="text-xs font-black text-blue-900 uppercase">Focus Area</h4>
                    <p className="text-xs text-blue-700 mt-1">High-value enterprise leads show maximum growth potential this quarter.</p>
                  </div>
                </div>
                <div className="bg-amber-50 p-4 rounded-2xl flex items-start gap-4">
                  <AlertTriangle className="text-amber-600 shrink-0" size={20} />
                  <div>
                    <h4 className="text-xs font-black text-amber-900 uppercase">Attention</h4>
                    <p className="text-xs text-amber-700 mt-1">Monitor manpower utilization to ensure service quality as leads scale.</p>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div className="bg-slate-50 px-8 py-4 flex items-center justify-between border-t border-slate-100">
          <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest flex items-center gap-2">
            <Zap size={10} className="text-blue-500" /> Powered by Gemini 3 Flash
          </p>
          <button 
            onClick={onClose}
